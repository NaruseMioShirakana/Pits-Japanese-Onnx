...
class StochasticDurationPredictor(nn.Module):
    ...
    def __init__(self,
        ...
        self.log_flow = opits_modules.Log()
        self.flows = nn.ModuleList()
        self.flows.append(opits_modules.ElementwiseAffine(2))
        for i in range(n_flows):
            self.flows.append(
                opits_modules.ConvFlow(2, filter_channels, kernel_size, n_layers=3))
            self.flows.append(opits_modules.Flip())

        self.post_pre = nn.Conv1d(1, filter_channels, 1)
        self.post_proj = nn.Conv1d(filter_channels, filter_channels, 1)
        self.post_convs = opits_modules.DDSConv(filter_channels,
                                          kernel_size,
                                          n_layers=3,
                                          p_dropout=p_dropout)
        self.post_flows = nn.ModuleList()
        self.post_flows.append(opits_modules.ElementwiseAffine(2))
        for i in range(4):
            self.post_flows.append(
                opits_modules.ConvFlow(2, filter_channels, kernel_size, n_layers=3))
            self.post_flows.append(opits_modules.Flip())

        self.pre = nn.Conv1d(in_channels, filter_channels, 1)
        self.proj = nn.Conv1d(filter_channels, filter_channels, 1)
        self.convs = opits_modules.DDSConv(filter_channels,
                                     kernel_size,
                                     n_layers=3,
                                     p_dropout=p_dropout)
        if gin_channels != 0:
    ...
    def forward(self, x, x_mask, zinput, g=None):
        x = torch.detach(x)
        x = self.pre(x)
        if g is not None:
            g = torch.detach(g)
...
class ResidualCouplingBlock(nn.Module):
    ...
    def forward(self, x, x_mask, g=None):
        ...
        z_spec, z_yin = torch.split(x, self.inter_channels - self.yin_channels, dim=1)
        z_yin_crop = z_yin[:, self.yin_start:self.yin_start + self.yin_scope, :]

        x = torch.cat([z_spec, z_yin_crop], dim=1) * x_mask
        return x
...
class SynthesizerTrn(nn.Module):
    ...
    def infer(self, x, t, x_lengths, sid=None, noise_scale=1, length_scale=1, noise_scale_w=1., max_len=None, scope_shift=0):  #need to fix #vector scope shift needed
        x, m_p, logs_p, x_mask = self.enc_p(x, t, x_lengths)
        if self.n_speakers > 0:
            g = self.emb_g(sid).unsqueeze(-1)  # [b, h, 1]
        else:
    ...
    def export_onnx(self, Onnx = False):  #need to fix #vector scope shift needed
        noise_scale=1
        length_scale=1
        noise_scale_w=1.
        scope_shift=0
        ...
        self.flow.yin_start = self.yin_start
        self.flow.yin_scope = self.yin_scope
